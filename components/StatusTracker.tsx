...
const StatusTracker: React.FC<StatusTrackerProps> = ({ appointment, setAppointment }) => {
  ...
  if (!appointment) {
    return (
      <div className="text-center py-20 bg-white dark:bg-[#1e293b] rounded-xl shadow-sm border dark:border-slate-700">
        <i className="fas fa-calendar-times text-6xl text-gray-200 dark:text-gray-700 mb-4"></i>
        <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">No active appointment</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-8">You haven't booked any services yet.</p>
        <button 
          onClick={() => navigate('/dashboard')}
          className="bg-blue-900 dark:bg-blue-700 text-white px-8 py-3 rounded-lg font-bold"
        >
          Go to Dashboard
        </button>
      </div>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="bg-white dark:bg-[#1e293b] p-8 rounded-xl shadow-lg border border-gray-100 dark:border-slate-700 overflow-hidden relative transition-colors">
        <div className="absolute top-0 right-0 p-4 opacity-10 dark:opacity-5">
          <i className="fas fa-clock text-9xl text-gray-800 dark:text-gray-100"></i>
        </div>

        <div className="flex justify-between items-start mb-8 relative z-10">
          <div>
            <span className="text-xs font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 px-3 py-1 rounded-full uppercase tracking-widest">
              Live Status
            </span>
          ...
          <div className="text-right">
            ...
            <p className="text-xl font-bold text-gray-800 dark:text-gray-100">{appointment.timeSlot}</p>
          </div>
        </div>
        ...
        <div className="relative mb-12 mt-16 px-4">
          ...
          <div 
            className="absolute top-1/2 left-0 h-1 bg-blue-900 dark:bg-blue-500 -translate-y-1/2 transition-all duration-1000"
            ...
            {statuses.map((s, idx) => (
              <div key={s} className="flex flex-col items-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center relative z-20 border-4 transition-all duration-500 ${
                  idx <= currentIndex ? 'bg-blue-900 dark:bg-blue-500 border-blue-100 dark:border-blue-900 text-white' : 'bg-white dark:bg-slate-800 border-gray-100 dark:border-slate-700 text-gray-300 dark:text-gray-600'
                }`}>
                  {idx < currentIndex ? (
                ...
                <span className={`absolute top-10 text-[10px] font-bold uppercase whitespace-nowrap tracking-tighter ${
                  idx <= currentIndex ? 'text-blue-900 dark:text-blue-400' : 'text-gray-300 dark:text-gray-600'
                }`}>
                  {s}
                </span>
              </div>
        ...
        <div className="mt-20 p-6 bg-gray-50 dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700">
          <div className="flex items-start space-x-4">
            <div className={`w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 animate-bounce ${
              appointment.status === 'Completed' ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400' : 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400'
            }`}>
              <i className={`fas ${
        ...
        {appointment.status !== 'Completed' && (
          <div className="mt-8 text-center text-[10px] text-gray-400 dark:text-gray-500 italic">
      ...
      <div className="grid grid-cols-2 gap-4">
        <button className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 text-gray-600 dark:text-gray-400 py-3 rounded-xl font-bold hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
          Reschedule
        </button>
        <button 
          onClick={() => {
          ...
          className="bg-red-50 dark:bg-red-900/10 text-red-600 dark:text-red-400 py-3 rounded-xl font-bold hover:bg-red-100 dark:hover:bg-red-900/20 transition-colors border dark:border-red-900/30"
        >
          Cancel Visit
        </button>
      </div>
